...
- **Elaborate Parsewords red-herring enhancement made results worse.** The June-11 `SYSTEM_PROMPT` in `utils/parsewordsGenerator.ts` told the LLM to add new dead-end triggers (indicator-role confusion, link/filler swaps, false compute ops, mandatory definition red herrings, a chain-of-thought `analysis` field). Output quality dropped. Reverted (2026-06-30) to basic instructions: keep the deterministic skeleton, and have the LLM ONLY append as many genuine wrong OPTIONS as possible to the existing first-stage replace/result triggers (no new triggers). First-stage synonym distractors are the main lever for throwing the player off.
