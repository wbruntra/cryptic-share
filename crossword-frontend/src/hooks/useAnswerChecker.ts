...
export function useAnswerChecker() {
  ...
  const claimWord = useCallback(async (clueNumber: number, direction: Direction) => {
    if (!sessionId) return
  ...
  }, [grid, answers, answersEncrypted, dispatch, claimWord])

  const checkAllAnswers = useCallback(() => {
    console.log('[useAnswerChecker] checkAllAnswers START')
    console.log('[useAnswerChecker] state check:', {
      hasEncrypted: !!answersEncrypted,
      encryptedAcross: answersEncrypted?.across ? Object.keys(answersEncrypted.across).length : 0,
      encryptedDown: answersEncrypted?.down ? Object.keys(answersEncrypted.down).length : 0,
      gridLen: grid.length,
  ...
  const getCurrentClueNumber = useCallback((r: number, c: number, direction: Direction): number | null => {
  ...
  return { checkCurrentWord, getCurrentClueNumber, checkAllAnswers }
}
